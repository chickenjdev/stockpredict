...
import { GROUP_BALANCING_BUYCARD_DATA } from '@/utils/constants';
...
interface AnalysisProps {
  dashboardBuycarData: any;
  dispatch: Dispatch<any>;
  loading: boolean;
}

const Analysis: React.FC<AnalysisProps> = (props) => {
  const { dispatch, dashboardBuycarData } = props;
  ...
  const useDataAnalysis = useCallback(
    (dashboard) => {
      ...
      const listFilterBalancing = listBalancing.filter((item: any) => {
        const { store } = item || {};
        return GROUP_BALANCING_BUYCARD_DATA.includes(store);
      });
  ...
  useEffect(() => {
    if (dispatch) {
      reqRef.current = requestAnimationFrame(() => {
        dispatch({
          type: 'dashboardBuycarData/getBalancing',
          params: {},
        });
      });
    }
  ...
  useLayoutEffect(() => {
    ...
    useDataAnalysis(dashboardBuycarData);

    return () => { };
  }, [dashboardBuycarData, useDataAnalysis]);
  return (
    <div>
...
export default connect(
  ({
    ...
    dashboardBuycarData,
    loading: loading.effects['dashboardBuycarData/getBalancing'],
  }),
)(Analysis);
